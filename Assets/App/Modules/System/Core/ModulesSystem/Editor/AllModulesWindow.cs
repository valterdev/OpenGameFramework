...
namespace OpenGameFramework.Editor
{
    ...
    public class AllModulesWindow : EditorWindow
    {
        ...
        [MenuItem(Constants.MenuModuleEditorAllModule)]
        static void Init()
        {
            ResetInfo();
        ...
        [MenuItem(Constants.MenuModuleEditorUpdatePreInit)]
        ...
        private void PopulateListView()
        {
            ...
            Action<VisualElement, int> bindItem = (e, i) =>
            {
                ...
                var btn = e.Q<Button>("exp_btn");
                
                btn.userData = i;
                btn.UnregisterCallback<ClickEvent>(ExportBtnCallback);
                btn.RegisterCallback<ClickEvent>(ExportBtnCallback);
            };
        ...
        public static ProjectInfo GetProjectManifest()
        {
            LoadProjectManifest();
            return s_projectInfo;
        }
        
        public static void LoadProjectManifest()
        {
        ...
        private void ExportBtnCallback(ClickEvent evt)
        {
            var curBtn = (Button)evt.currentTarget;
            int curModuleID = (int)curBtn.userData;
            
            ExportModuleToFolder(curModuleID);
        }
        ...
        private void ActivateDeactivateModule(int curModuleID, bool value)
        {
            ...
            if (value == true)
            {
                ...
                {
                    if (s_projectInfo.not_active_modules[i].hash != s_findedModules[curModuleID].hash) continue;
                ...
                // Delete hooks from DisabledHooks directory
                var hooksForDelete = s_projectInfo.disabled_hooks
                    .Select(hook => hook.path)
                    .ToArray();
                DeleteDisabledHooks(hooksForDelete);
            } else
            {
                ...
                // add info about disabled hooks
                var moduleDirectory = Path.Combine(Constants.Assets, s_findedModules[curModuleID].GetModulePath());
                var hookFiles = FindDisabledHooks(moduleDirectory);
                s_projectInfo.disabled_hooks = hookFiles
                    .Select(hookPath => new DisabledHooksInfo()
                    {
                        path = Path.Combine(Application.dataPath, "App/Modules/System/Config/DisabledHooks/", hookPath.Name)
                    })
                    .ToList();
                ...
                // Save hooks if module have it.
                SaveDisabledHooks( hookFiles );
        ...
        public static void SaveProjectManifest()
        {
        ...
        private void ActivateDeactivateMode()
        {
            ...
            if (_activateMode)
            {
                ...
                _mainPage.Query<Button>().Class("exp-btn").ForEach((elem) => {
                    elem.Hide();
                });
            } else
            {
        ...
        private static void ExportModuleToFolder(int moduleID)
        {
            var outPath = EditorUtility.SaveFilePanel(Constants.L_ExportModuleWindowTitle, "", s_findedModules[moduleID].name, "unitypackage");
            
            if (outPath == String.Empty) return;

            var moduleDirectory = Path.Combine(Constants.Assets, s_findedModules[moduleID].GetModulePath());

            ExportModuleToUnityPackage(moduleID, moduleDirectory, outPath);
        }
        ...
        private static FileInfo[] FindDisabledHooks(string modulePath)
        {
            ...
            try
            {
                files = root.GetFiles("*?Hooks.cs", SearchOption.AllDirectories);
            }
            catch (UnauthorizedAccessException e)
            {
                Debug.Log(e.Message);
        ...
        private static void SaveDisabledHooks(FileInfo[] files)
        {
            foreach (var file in files)
            {
                File.Copy(file.FullName, Path.Combine(Application.dataPath, "App/Modules/System/Config/DisabledHooks/", file.Name));
            }
        }

        private static void DeleteDisabledHooks(string[] files)
        {
            foreach (var filePath in files)
            {
                File.Delete(Path.Combine(Application.dataPath, "App/Modules/System/Config/DisabledHooks/", filePath));
            }
        }
