...
namespace OpenGameFramework.Editor
{
    ...
    public class CreateModuleWindow : EditorWindow
    {
        ...
        [MenuItem(Constants.MenuModuleEditorCreateModule)]
        static void Init()
        {
            if (_moduleInfo == null)
            {
        ...
        private void CreateModule()
        {
            ...
            var serviceName = _moduleInfo.custom_manager_name == string.Empty ? _moduleInfo.name + Constants.Service : _moduleInfo.custom_manager_name;

            string[] fileNames = {
                Constants.ModuleInfoFileName,
                _moduleInfo.name + "SingletonParts.cs",
                managerName + ".cs",
                _moduleInfo.name + "Hooks.cs",
                _moduleInfo.name + "Store.cs",
                serviceName + ".cs",
                "I" + _moduleInfo.name + "SystemService.cs",
                "I" + _moduleInfo.name + "DomainService.cs",
                _moduleInfo.name + "Config.cs"
                };
            ...
            if (!Directory.Exists(Path.Combine(Application.dataPath, filePath)))
            {
                ...
                {
                    // Parts
                    // WriteFileBaseOnTemplate(Constants.PathToSingletonPartsScriptTemplate, filePath, fileNames[1], managerName);

                    // Singleton Manager
                    WriteFileBaseOnTemplate(Constants.PathToSingletonManagerScriptTemplate, filePath, fileNames[2], managerName);
                }
                else
                {
                    // DI Service
                    WriteFileBaseOnTemplate(Constants.PathToServiceScriptTemplate, filePath, fileNames[5], serviceName);
                    
                    // System Interface for API
                    WriteFileBaseOnTemplate(Constants.PathToAPISystemInterfaceTemplate, filePath, fileNames[6], serviceName);
                    
                    // Domain Interface for API
                    WriteFileBaseOnTemplate(Constants.PathToAPIDomainInterfaceTemplate, filePath, fileNames[7], serviceName);
                }
                ...
                // Config For Module
                WriteFileBaseOnTemplate(Constants.PathToConfigSOTemplate, filePath, fileNames[8], _moduleInfo.name);
                
                //CreateModuleConfig(filePath, );
                ProjectInfo info = AllModulesWindow.GetProjectManifest();
                info.config_for_generate = $"{filePath}--{_moduleInfo.name}";
        ...
        private static void CreateModuleConfig()
        {
            ...
            if (info.config_for_generate == "") return;

            var configInfo = info.config_for_generate.Split("--");
            info.config_for_generate = "";
            
            AllModulesWindow.SaveProjectManifest();
            
            string filePath = configInfo[0];
            string moduleName = configInfo[1];
            Assembly[] assemblies = System.AppDomain.CurrentDomain.GetAssemblies();
            Type configType = typeof(ModuleConfig);
            
            foreach (var assembly in assemblies)
            {
                var classTypes = assembly
                    .GetTypes()
                    .Where(x => x.IsClass && x.Namespace == "OpenGameFramework")
                    .Select(x => x).ToArray();
                    
                foreach (var type in classTypes)
                {
                    if (type.Name == $"{moduleName}Config")
                    {
                        configType = type;
                    }
                }
            ...
            string path = Path.Combine(Constants.Assets, filePath, moduleName + "_Config.asset");
